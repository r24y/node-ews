...
var httpntlm = require('httpntlm-3');
...
EWS.prototype._validate = function(callback) {
  ...
  function getFile(url, file, callback) {
    httpntlm.get({ url: url, domain: $this.domain, username: $this.username, password: $this.password, rejectUnauthorized: false },
    function(err, res) {
      if(err) {
        callback(err);
      } else {
    ...
    if(err) {
      ...
      soap.createClient($this.ewsSoap[0].path, {httpClient: HttpClient}, function(err, client) {
        if(err) {
          ...
          client.setSecurity(new NtlmSecurity($this.username, $this.password, {strictSSL: false, rejectUnauthorized: false, domain: $this.domain}));
